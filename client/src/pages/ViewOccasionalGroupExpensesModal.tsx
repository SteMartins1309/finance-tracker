...
import { FileText, Tags, Calendar, Star, Repeat2 } from "lucide-react"; // Importar Repeat2
...
// Importando as interfaces e funções auxiliares do types/finance.ts, já que o problema foi resolvido
import { RecentExpense } from "@/types/finance"; // Ajustado para importar de types/finance.ts

// Funções auxiliares (estas funções DEVEM ser exportadas de um arquivo utilitário comum
// se você não as tiver duplicadas em cada componente. Para este exemplo, estou mantendo
// as definições aqui, mas a melhor prática é tê-las em um `src/lib/utils.ts` ou similar e importá-las.)
...
export function ViewOccasionalGroupExpensesModal({
  ...
  const [isExporting, setIsExporting] = useState(false); // Para desabilitar o botão durante a exportação
  ...
  const totalAmount = expenses.reduce((sum: number, expense: RecentExpense) => sum + parseFloat(expense.amount), 0);

  // Agrupa as despesas por routineCategory para os acordeões
  const groupedExpenses = expenses.reduce((acc: Record<string, RecentExpense[]>, expense: RecentExpense) => {
    const category = expense.routineCategory || 'outros';
    if (!acc[category]) {
      acc[category] = [];
    }
  ...
  const sortedCategories = Object.keys(groupedExpenses).sort((a: string, b: string) => {
  ...
  const handleExportPdf = async () => {
    if (!modalContentRef.current || !expenses || expenses.length === 0) {
      toast({
        title: "Erro",
        description: "Não há gastos ou conteúdo para exportar.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      // 1. Abrir todos os acordeões de categoria para que o conteúdo seja renderizado
      const allCategoryKeys = sortedCategories;
      setExpandedAccordionItems(allCategoryKeys);

      // 2. Aguardar a renderização dos acordeões abertos
      await new Promise(resolve => setTimeout(resolve, 500)); // Ajuste este tempo se necessário
      ...
      // 3. Voltar os acordeões ao estado colapsado após a exportação
        ...
        {isLoading ? (
          ...
          <div ref={modalContentRef}>
            <div className="flex justify-between items-center mb-4 p-2 bg-gray-50 rounded-lg">
              ...
              >
                <FileText className="mr-2 h-4 w-4" /> {isExporting ? "Gerando PDF..." : "Exportar PDF"}
              </Button>
            </div>
            ...
            {expenses.length === 0 ? (
              ...
              <Accordion
                type="multiple" // Use 'multiple' para permitir que vários acordeões fiquem abertos
                value={expandedAccordionItems} // Controlado pelo estado `expandedAccordionItems`
              ...
              >
                {sortedCategories.map((category) => {
                  ...
                  const categoryTotal = categoryExpenses.reduce((sum: number, exp: RecentExpense) => sum + parseFloat(exp.amount), 0);

                  categoryExpenses.sort((a: RecentExpense, b: RecentExpense) => new Date(b.purchaseDate).getTime() - new Date(a.purchaseDate).getTime());
                  ...
                  return (
                    <AccordionItem value={category} key={category} className="border-b">
                      ...
                      <AccordionContent className="space-y-1">
                        {categoryExpenses.map((expense: RecentExpense, index: number) => {
                          const { details, description: expenseDescription } = getSubcategoryDetails(expense);
                          const iconBgColor = getCategoryIconColor(expense.routineCategory || 'outros');
                          const iconToDisplay = getCategoryIcon(expense.routineCategory || 'outros');
